        ...
        <div class="bg-gradient-to-r from-green-100 to-green-200 rounded-lg shadow-lg p-6 text-gray-900">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach ($section['items'] as $item)
              @if ($item['url'] != '#')
                <a target="_blank" href="{{ asset($item['url']) }}"
              ...
              @else
                <div class="block bg-white bg-opacity-60 rounded-lg p-4 hover:bg-opacity-80 hover:scale-105 hover:shadow-lg transition-all duration-300 ease-in-out transform">
                  <h4 class="text-gray-800 font-semibold hover:text-gray-900 transition-colors">{{ $item['title'] }}</h4>
                  <p class="text-sm text-gray-600 mt-1">{{ $item['description'] }}</p>
                </div>
              @endif
